...
// план такой, выделять память на вот эту структуру - 16 байт минимально
// потом сделать общий массив, потом сделать функцию добавление в свободный лист
// и проработать логику с этим

header_t *free_blocks[10] = {NULL};

typedef struct free_block_t {
  header_t prev_free_block;
  header_t next_free_block;
} free_block_t;
